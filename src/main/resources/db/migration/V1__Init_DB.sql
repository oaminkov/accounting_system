...
alter table user_role add constraint user_role__usr_fk foreign key (user_id) references usr
...
alter table organization add constraint organization__country_fk foreign key (id_country) references country
    ON DELETE CASCADE ON UPDATE CASCADE;
alter table uploaded_file add constraint uploaded_file__inf_res_fk foreign key (information_resource) references information_resource
    ON DELETE CASCADE ON UPDATE CASCADE;
